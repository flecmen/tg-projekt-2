...
class SeznamSousedu<T> {
    ...
    getNodesNeighboursCount(options?: { limit?: number, sort?: 'asc' | 'desc', direction?: 'in' | 'out' | 'both' }) {
        ...
        if (direction === 'in' || direction === 'both') {
            const incomingMap: Map<string, number> = new Map();
    ...
    getMissingDirectionalEdges(): [T, T][] {
        const missingEdges: [T, T][] = [];
